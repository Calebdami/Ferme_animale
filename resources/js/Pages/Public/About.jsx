    ...
    return (
        <PublicLayout>
            ...
            <Container className="py-14 sm:py-20">
                ...
                <div className="prose prose-invert mt-10 max-w-2xl whitespace-pre-line text-sand-300">
                    {page?.content}
                </div>

                {team.length > 0 && (
                    <div className="mt-14">
                        <h2 className="mb-6 font-display text-xl font-semibold text-sand-100">Notre équipe</h2>
                        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                            {team.map((m) => (
                                <div key={m.id} className="overflow-hidden rounded-xl border border-soil-700 bg-soil-900/60">
                                    <div className="aspect-square overflow-hidden bg-soil-800">
                                        <img src={m.url} alt={m.alt_text || m.title} className="h-full w-full object-cover" />
                                    </div>
                                    {m.title && <p className="p-2 text-center text-xs text-sand-300">{m.title}</p>}
                                </div>
                            ))}
                        </div>
                    </div>
