...
export default function Home({ settings, activities, testimonials, news, featuredPoultry }) {
    return (
        <PublicLayout>
            ...
            {/* Hero */}
            <section className="relative overflow-hidden border-b border-gray-100 dark:border-soil-800">
                <Container className="grid gap-10 py-16 sm:py-20 lg:grid-cols-2 lg:items-center lg:py-28">
                    <div>
                        ...
                        <h1 className="font-display text-4xl font-semibold leading-[1.1] text-gray-900 dark:text-sand-100 sm:text-5xl">
                            {settings.hero_title || "Une volaille d'exception, du couvoir jusqu'à votre élevage"}
                        </h1>
                        <p className="mt-5 max-w-lg text-gray-500 dark:text-sand-400">
                        ...
                        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                            ...
                            <Link href={route('contact')} className="rounded-lg border border-gray-300 dark:border-soil-700 px-5 py-3 text-center text-sm font-medium text-gray-700 dark:text-sand-100 hover:border-yolk-500">
                                Nous contacter
                            </Link>
                        </div>
                    </div>
            ...
            <section className="py-16 sm:py-20">
                <Container>
                    <SectionTitle eyebrow="Ce que nous faisons" title="Nos activités" subtitle="De l'élevage à la vente, en passant par le dépistage sanitaire, chaque étape est suivie avec rigueur." />
                    <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {activities.map((activity) => (
                            <ActivityCard key={activity.id} activity={activity} />
                        ))}
                    </div>
                    <Link href={route('activities')} className="mt-8 inline-block text-sm text-yolk-500 hover:underline">
            ...
            {featuredPoultry.length > 0 && (
                <section className="border-t border-gray-100 dark:border-soil-800 py-16 sm:py-20">
                    <Container>
                        <SectionTitle eyebrow="Catalogue" title="Races disponibles" subtitle="Un aperçu de nos poussins et volailles actuellement disponibles." />
                        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            {featuredPoultry.map((p) => (
                                <PoultryCard key={p.id} poultryType={p} />
                            ))}
                        </div>
                        <Link href={route('poultry.index')} className="mt-8 inline-block text-sm text-yolk-500 hover:underline">
            ...
            {/* CTA */}
            <section className="border-t border-gray-100 dark:border-soil-800 py-16 sm:py-20">
                <Container className="rounded-2xl border border-yolk-500/30 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-soil-900 dark:to-soil-800 p-8 text-center sm:p-12">
                    <h2 className="font-display text-2xl font-semibold text-gray-900 dark:text-sand-100 sm:text-3xl">
                        Prêt à passer commande ?
                    </h2>
                    <p className="mx-auto mt-3 max-w-md text-gray-500 dark:text-sand-400">
